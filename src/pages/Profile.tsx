      ...
      <section className="parchment rounded-[28px] border border-amber-100/70 p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-2xl font-black text-slate-950">Magische Preise</h2>
          </div>
        </div>
          ...
          {rewards.map(reward => (
            <div key={reward.id} className={`flex items-center justify-between gap-4 rounded-2xl border border-amber-900/10 bg-white/60 p-4 ${reward.claimed ? 'opacity-60' : ''}`}>
              <div className="flex items-center gap-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-950 text-2xl text-amber-100">
                  {reward.icon}
                </div>
                <div>
              ...
              {reward.claimed ? (
                <span className="rounded-full bg-blue-100 px-3 py-2 text-xs font-black uppercase tracking-[0.12em] text-blue-800">Erhalten</span>
              ) : (
                <button
