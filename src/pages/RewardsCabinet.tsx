...
import { CheckCircle2, Gift, LockKeyhole, PackageCheck, Sparkles } from 'lucide-react';
    ...
    <main className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-[1500px] gap-4 px-3 py-3 sm:px-4 sm:py-4 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)]">
      <section className="ink-panel rounded-[24px] border border-amber-100/20 p-4 text-amber-50 xl:rounded-[28px] xl:p-6">
        <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-amber-200 text-slate-950 xl:h-24 xl:w-24 xl:rounded-[28px]">
          <Gift className="h-9 w-9 xl:h-12 xl:w-12" />
        </div>
        <div className="mt-5 text-xs font-black uppercase tracking-[0.18em] text-amber-200/70">Belohnungsschrank</div>
        <h1 className="mt-2 text-2xl font-black xl:text-3xl">Magische Fächer</h1>
        <p className="mt-3 text-sm font-semibold leading-6 text-amber-50/75 xl:mt-4">
          Hier liegen Spiel-Belohnungen und echte Ziele. Manche Fächer öffnen sich mit Wortfunken, andere erst nach einem besonderen Level.
        </p>

        <div className="mt-4 rounded-2xl border border-amber-100/20 bg-white/10 p-4">
          <div className="flex items-center gap-2 text-amber-200">
            <Sparkles className="h-5 w-5" />
            <span className="text-3xl font-black">{sparkBalance}</span>
          </div>
          <div className="mt-2 text-xs font-black uppercase tracking-[0.14em] text-amber-100/60">Wortfunken</div>
        </div>

        {message && (
          <div className="mt-4 rounded-2xl bg-blue-100 px-4 py-3 text-sm font-black leading-5 text-blue-950">
            {message}
          </div>
        )}
      </section>
